...
train_model <- function(data) {
  lm(hosp_rate_target ~ wastewater_current + wastewater_change_2w + 
...
main <- function() {
  ...
  saveRDS(lm_model, 
          "linear_regression/output/covid_prediction_model.rds")
  write_csv(model_data_lagged, 
            "linear_regression/output/model_data.csv")
...
#   Null check for safety
  if (nrow(model_data_2025) > 0) {
    ...
    if (nrow(model_data_2025_lagged) > 0) {
      ...
      png("linear_regression/analysis_output/2025_prediction_comparison.png", 
      ...
      write_csv(results_summary, 
                "linear_regression/analysis_output/model_accuracy_metrics.csv")
      ...
      write_csv(predictions_df, 
                "linear_regression/analysis_output/2025_predictions.csv")
    }
  }
  ...
  write_csv(coef_df, 
            "linear_regression/analysis_output/model_coefficients.csv")
}
